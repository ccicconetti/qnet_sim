...
#[derive(Debug, PartialEq, Eq)]
pub struct EprEntangleSwappingData {
    /// Predecessor EPR pair ID.
    pub epr_pair_id_pred: u64,
    /// Successor EPR pair ID.
    pub epr_pair_id_succ: u64,
    /// Repeater node ID.
    pub bsm_node_id: u32,
}
...
pub enum NetworkEventData {
    ...
    /// Free a half EPR pair, without measuring the fidelity.
    EprFree(EprFreeData),
    /// Entangle two EPR pair.
    EprEntanglementSwapping(EprEntangleSwappingData),
}
...
impl Event {
    ...
    pub fn immediate(event_type: EventType) -> Self {
        Self {
            time: 0,
            event_type,
            transfer: None,
